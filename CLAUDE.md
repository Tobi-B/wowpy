...
- **Block programming (`wowpy/blocks.py`, `codegen.py`, `program.py`, `storage.py`):** `blocks.py` describes every editor block once — its Blockly definition *and* the Python it generates (`{FIELD}`, `{FIELD!r}`, `{FIELD!rgb}`, `{$input}`, `{^body}`, `{id}`, `{id!name}`). `codegen.py` turns a saved workspace into a module with `async def main(mip)`, event handlers, custom-block functions and Blockly `procedures_*` functions (all sharing one collision-free name space; variable references are resolved through the workspace's `variables` table, since Blockly saves only the id); `program.py` executes it (event hats, `sensors`, block highlighting via `_step`, stop/cancel that always halts the robot); `storage.py` reads and writes `programs/*.json` + `.py` and `blocks/*.json`. Add a block in `blocks.py`, not in the page.
- **Python blocks (`wowpy/blockcode.py`, US-005):** a custom block may carry `"mode": "python"` and a hand-written `code` body instead of a workspace; a converted block keeps its `workspace` as provenance so it can be regenerated. `blockcode.validate()` runs before anything is written and rejects two things: code that will not compile as the body of `async def <block>(mip, ...)`, and **any loop without an `await`** - generated loops always yield, and a hand-written `while True: pass` would freeze the server's event loop, taking Stop and every open page with it. Because a hand-written body can call another block's function by name, `codegen` emits **every** defined custom block, used or not. `Store.save_block()` keeps fields it does not manage (e.g. `shipped`), so a round trip does not drop them.
