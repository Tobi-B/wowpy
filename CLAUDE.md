...
`wowpy` is a Python package for simple robot control over Bluetooth LE, starting with the WowWee MiP robot, with planned Blockly bindings. `wowpy/mip.py` holds the `MiP` class (discovery, connection, commands, status query), `wowpy/protocol.py` the full opcode table, `wowpy/dashboard/` a browser dashboard for testing every command; `main.py` is a small usage example.
